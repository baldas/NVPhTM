...
void NVMHTM_init_thrs(int nb_threads)
{
  ...
  *NH_checkpointer_state = 0;
  ...
  key = KEY_CHKP_AVG_TIME;
  shmid = shmget(key, sizeof (unsigned long long), 0777 | IPC_CREAT);
  shmctl(shmid, IPC_RMID, NULL);
  shmid = shmget(key, sizeof (unsigned long long), 0777 | IPC_CREAT);
  if (shmid < 0) {
    perror("shmget CHKP_AVG_TIME");
  }

  NH_time_checkpoints_average = (unsigned long long *) shmat(shmid, (unsigned long long *) 0, 0);

  if (NH_time_checkpoints_average == 0) perror("shmat");
  *NH_time_checkpoints_average = 0;
...
static void fork_manager()
{
  ...
  *NH_checkpointer_state = 1; // BUSY
  ...
  if (pid == 0) {
    ...
    if (MAX_PHYS_THRS == 72) {
      set_affinity_at(53); // last hyperthread of first socket
    } else {
...
//    printf("APPLY_LOG: ");
chkp_return_value = 1; // necessary if the following test fails
#if SORT_ALG == 1
if (LOG_is_logged_tx()) {
  #else
  if (LOG_is_logged_tx2()) {
    ...
    #elif SORT_ALG == 5
    chkp_return_value = LOG_checkpoint_backward();
    #endif
    // LOG_move_start_ptrs();
  ...
  // only consider time when the checkpoint really happened
  if (chkp_return_value == 0) {
    time_chkp_2 = rdtscp();

  // time_o_chkp_1 = rdtscp();

  //NH_nb_checkpoints++; -> this is happening in the log function
    time_chkp_total += time_chkp_2 - time_chkp_1;
 
  //if (NH_nb_checkpoints > 1)
    *NH_time_checkpoints_average = time_chkp_total / NH_nb_checkpoints;
  }
...
static void segint_sigaction(int signal, siginfo_t *si, void *context)
{
  ...
  NH_nb_checkpoints);
  ptr += sprintf(ptr, "[FORKED_MANAGER] Time active %f ms - average %f ms!\n",
  (double) time_chkp_total / ((double) CPU_MAX_FREQ), (double) *NH_time_checkpoints_average / (double) CPU_MAX_FREQ );
