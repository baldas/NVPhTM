...
int some_array[64][128];
__thread int some_array_idx;
...
void NVHTM_thr_init()
{
    ...
    // should be done only once on system startup, not here
#if 0
	  NH_tx_time_total = 0.0;
  	NH_time_after_commit_total = 0.0;
	  NH_tx_time = 0;
	  NH_time_after_commit = 0;
    NH_time_blocked_total = 0;
	  NH_count_writes_total = 0;  
	  NH_count_blocks_total = 0;
	  NH_time_validate_total = 0;
#endif
...
void NVHTM_shutdown()
{
    ...
    printf("TOTAL_TIME_B (clocks) %llu %f ms\n", NH_time_blocked_total, (double) NH_time_blocked_total / (double) CPU_MAX_FREQ);
    printf(" ---   ----\n");
    printf(" ########################################### \n");
    printf(" ########################################### \n\n");
