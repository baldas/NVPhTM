...
#define FORCE_CHECKPOINT  \
			/*sem_post(NH_chkp_sem);*/ \
      int success; \
      int expected = 0; \
			while (! (success = __atomic_compare_exchange_n(NH_checkpointer_state, \
					 &expected,2,/*not weak*/0, __ATOMIC_SEQ_CST, __ATOMIC_ACQUIRE))) { \
				if (expected == 2) break;\
				expected = 0; \
				_mm_pause(); \
			}\
      if (expected == 2 || success) sem_post(NH_chkp_sem); \
			while (__atomic_load_n(NH_checkpointer_state, __ATOMIC_SEQ_CST) != 0) { \
				_mm_pause(); \
			}
...
#define IF_HTM_MODE							GET_TX_ID(); \
                                while(1){ \
																	uint64_t mode = getMode(); \
																	if (mode == HW || mode == GLOCK){
#define START_HTM_MODE 							BEFORE_TRANSACTION(__tid__, 0 /* unused */); \
                                    bool modeChanged = HTM_Start_Tx(); \
																		if (!modeChanged) {
#define COMMIT_HTM_MODE								BEFORE_COMMIT(__tid__, 0 /* unused */, HTM_SUCCESS); \
                                      HTM_Commit_Tx(); \
                                      AFTER_TRANSACTION(__tid__, 0 /* unused */); \
																			break; \
																		}
#define ELSE_STM_MODE							} else {
/*#define START_STM_MODE(ro)			    FORCE_CHECKPOINT \ */
#define START_STM_MODE(ro)			    jmp_buf _jmpbuf; \
																		uint32_t abort_flags = setjmp(_jmpbuf); \
																		bool restarted = abort_flags != 0; \
																		bool modeChanged = STM_PreStart_Tx(restarted); \
...
#define HW_TM_SHARED_READ(var)        \
	NVM_HW_READ_BARRIER(var)
#define HW_TM_SHARED_READ_P(var)      \
	NVM_HW_READ_BARRIER(var)
#define HW_TM_SHARED_READ_F(var)      \
	NVM_HW_READ_BARRIER(var)
...
#define HW_TM_SHARED_WRITE(var, val)   \
	NVM_HW_WRITE_BARRIER(var, val)
#define HW_TM_SHARED_WRITE_P(var, val) \
	NVM_HW_WRITE_BARRIER(var, val)
#define HW_TM_SHARED_WRITE_F(var, val) \
	NVM_HW_WRITE_BARRIER(var, val)
